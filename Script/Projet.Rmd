...
#on ne prendras pas en compte la varible hotel car tres correlee avec la population
```
...
```{r}
#pooled
reg_pooled1 = lm(EQI_norm ~ . ,  data=data_p)
summary(reg_pooled1)
```
...
reg_heter <- lm(EQI_norm ~ pop:annee + surface:annee + revenu:annee + emploi:annee + meurtre:annee + education:annee - 1, data = data)

summary(reg_heter)


#methode Zellner ou SURE
#regarder covariance
...
#modele a effets fixes indiviuels
#exemple surface du pays : n'evolue pas dans le temps, coefficient ne bouge pas -> inclut dans la constante

#dans notre cas pas besoin d'estimer un coefficient associe a la surface pour chaque annee

#il faut creer N variables indicatrices
#= 1 si correspond a l'individu et 0 sinon

#s'estime par MCO si les residus sont homoscedastiques et non autocorreles
#= Least Squar Dummy Variable

#si N grand -> estimateur within : MCO sur donnees centrees
 

#modele a effets individuels aleatoires
